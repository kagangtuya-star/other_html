...
**进程间通信 (IPC)** 是进程之间相互通信的技术，一般有如下几种

![image-20230704163308457](UNIX编程·进程与线程.assets/image-20230704163308457.png)

`UNIX系统IPC`是各种进程通信方式的统称，但是，这些通信方式中极少有能在所有UNIX系统实现中进行移植的。随着`POSIX`和`TheOpenGroup`（以前是X/Open）标准化的推进和影响的扩大，情况已得到改善，但差别仍然存在。

本章将讨论管道、FIFO、消息队列、信号量、共享存储这几种进程通信方式。

此外，消息队列、信号量以及共享存储器还被并称为`XSI IPC`，有许多相似之处，将在后面说明。

### 管道

#### 概念

管道顾名思义，即在进程之间打开一条通道。这是UNIX系统IPC的最古老形式，所有UNIX系统都提供此种通信机制，但它们有着如下局限性：

- 部分系统只支持半双工的管道（即数据只能在一个方向上流动）。现在，某些系统提供全双工管道，但是为了最佳的可移植性，我们决不应预先假定系统支持全双工管道。
- 管道只能在具有一个父进程的两个进程之间使用。通常，一个管道由一个进程创建，在进程调用fork之后，这个管道就能在父进程和子进程之间使用了。

后文提到的`FIFO`没有第二种局限性。

尽管有这两种局限性，半双工管道仍是最常用（也是移植性最好）的IPC形式。例如，每当在管道中键入一个命令序列，让 `shell` 执行时，`shell` 都会为每一条命令单独创建一个进程，然后用管道将前一条命令进程的标准输出与后一条命令的标准输入相连接。
...
`fd`：长度为 2的整数数组，用于存储新创建的管道的两个文件描述符。`fd[0]` 用于从管道读取数据，`fd[1]` 用于向管道写入数据。`fd[1]`的输出是`fd[0]`的输入。
...
![image-20230704163630708](UNIX编程·进程与线程.assets/image-20230704163630708.png)

单个进程中的管道几乎没有任何用处 。通常，进程会先调用 **`pipe`** ，接着 **`fork`** ，从而创建从父进程到子进程的 **IPC `通道`** 。对于父进程到子进程的管道 ，父进程关闭管道的读端 `fd[0]` ，子进程关闭写端 `fd[1]`（如下图所示）。子进程到父进程的管道反之 。

一个管道通常只有一个读进程和一个写进程。如果管道的一侧被关闭，则
...
#### 创建管道连接进程，并发送数据——`popen` / `pclose`函数

它的作用是创建一个连接到另一个进程的管道，然后读其输出或向其输入端发送数据
...
//例子 fp = popen("cmd 2>&1", "r");
```

**参数**

`cmdstring`：要执行的 shell 命令字符串。

`type`：文件指针的类型，可以是 "r"（只读）或 "w"（只写）。

`fp`：由 popen 返回的文件指针。

**返回值**

- `popen` 函数返回一个文件指针，如果执行命令失败，则返回 NULL。
- `pclose` 函数返回命令的退出状态，如果关闭文件指针失败，则返回 -1。

**原理**

`popen`先执行 **`fork`** 创建子进程，然后调用 **`exec`** 执行 `cmdstring` ，并且返回一个标准 I/O 文件指针 。如果 type 是 `r` ，则文件指针连接到 `cmdstring` 的标准输出（ 返回的文件指针是可读的 ）；若 type 是`w` ，则文件指针连接到 `cmdstring` 的标准输入（ 返回的文件指针是可写的 ）

**`pclose`** 函数关闭标准 I/O 流，等待命令终止，然后返回 shell 的终止状态。如果 shell 不能执行，则 **`pclose`** 返回值的终止状态与 shell 已经执行 **`exit(127)`** 一样 。

*在Unix/Linux系统中，如果一个进程尝试执行一个不存在的命令，那么这个进程会收到一个 SIGCHILD 信号，并调用 exit(127) 函数终止。在这种情况下，pclose 函数返回的终止状态也会是 127。*

#### 工具人之协同进程——管道的一种用法

`UNIX`系统过滤程序从标准输入读取数据，向标准输出写数据。

几个过滤程序通常在shell管道中线性连接。当一个过滤程序既产生某个过滤程序的输入，又读取该过滤程序的输出时，它就变成了协同进程（`coprocess`）。协同进程通常在`shell`的后台运行，其标准输入和标准输出通过管道连接到另一个程序。

虽然初始化一个协同进程，并将其输入和输出连接到另一个进程的`shell`语法是十分奇特的，但是协同进程的工作方式在C程序中也是非常有用的。

`popen`只提供连接到另一个进程的标准输入或标准输出的一个单向管道，而协同进程则有连接到另一个进程的两个单向管道：一个接到其标准输入，另一个则来自其标准输出。我们想将数据写到其标准输入，经其处理后，再从其标准输出读取数据。

![image-20230704170849483](UNIX编程·进程与线程.assets/image-20230704170849483.png)

### FIFO

`FIFO`有时被称为`命名管道`。未命名的管道只能在两个相关的进程之间使用，而且这两个相关的进程还要有一个共同的创建了它们的祖先进程。但是，通过`FIFO`，**不相关的进程也能交换数据。**

#### 创建 `FIFO` ——`mkfifo/mkfifoat`函数
...
`mode`：指定文件权限的模式，通常使用 0666。

**返回值**

- 成功创建了命名管道，则返回 0。
- 失败，则返回 -1，并设置 `errno` 错误码

**`mkfifo`** 函数中的 mode 参数的规格说明与 `**open**` 函数中 `mode` 的相同 ，但**`mkfiat`** 函数可以被用来在 `fd` 文件描述符表示的目录相关的位置创建一个 **FIFO** 。

这里有3种情形

- 如果`path`参数指定的是绝对路径名，则fd参数会被忽略掉，并且`mkfifoat`函数的行为和`mkfifo`类似。
- 如果`path`参数指定的是相对路径名，则`fd`参数是一个打开目录的有效文件描述符，路径名和目录有关。
- 如果path参数指定的是相对路径名，并且`fd`参数有一个特殊值`AT_FDCWD`，则路径名以当前目录开始，`mkfifoat`和`mkfifo`类似。

当我们用`mkfifo`或者`mkfifoat`创建`FIFO`时，要用`open`来打开它（其实正常的文件I/O函数（如close、read、write和unlink）都需要FIFO），当`open`一个`FIFO`时，非阻塞标（`O_NONBLOCK`）会产生下列影响。
...
类似于管道，若`write`一个尚无进程为读而打开的`FIFO`，则产生信号`SIGPIPE`。若某个`FIFO`的最后一个写进程关闭了该FIFO，则将为该FIFO的读进程产生一个文件结束标志。
...
### `XSI IPC`（即消息队列、信号量以及共享存储）

#### 总览

消息队列、信号量以及共享存储。

每个内核中的 IPC 结构（消息队列、信号量或共享存储段）都用**一个非负整数的标识符**（`identifier`）加以引用。例如，要向一个消息队列发送消息或者从一个消息队列取消息，只需要知道其队列标识符。
...
**让客户进程和服务进程在同一 IPC 结构上汇聚**：

- 服务器进程可以指定键 `IPC_PRIVATE` 创建一个新 **IPC** 结构，将返回的标识符存放在某处（ 如一个文件 ）以便客户进程取用 。缺点是：文件系统操作需要服务器进程将整型标识符写到文件中，此后客户进程又要读这个文件取得此标识符
- 客户进程和服务器进程认同一个路径名和 `项目ID` (0~255之间的字符值) ，接着调用 `ftok` 将这两个值变换为一个 键 ，然后服务器进程特定此键创建一个新的 IPC 结构 。

#### **流控制**

如果系统资源（如缓冲区）短缺，或者如果接收进程不能再接收更多消息，则发送进程就要休眠；当流控制条件消失时，发送进程应自动唤醒 。

#### 由一个路径名和`项目ID`产生一个键——**通用的`ftok`** 函数
...
`path`：一个字符串，用于生成 IPC 键值的路径名。这个路径名必须是一个存在的文件或目录，否则会失败。

`id`：一个整数，用于生成 IPC 键值的项目 ID。这个 ID 通常是一个非常小的正整数，通常取值为 0。

**返回值**

- 成功，返回一个 System V IPC 键值。
- 失败，则返回 -1，并设置 errno 错误码。

对应的3 个 **`get`** 函数（ **`msgget`** 、**`semget`** 和 **`shmget`** ）都有两个类似的参数：一个 `key` 和一个整型 `flag` ，规则为：
...
在创建IPC结构时，对所有字段都赋初值。以后，可以调用`msgctl`、`semctl`或`shmctl`修改`uid`、`gid`和`mode`字段。为了修改这些值，调用进程必须是IPC结构的创建者或超级用户。修改这些字段类似于对文件调用`chown`和`chmod`。

`mode`字段的值类似于下图中所示的值，但是对于任何IPC结构都不存在执行权限。另外，消息队列和共享存储使用术语“读”和“写”，而信号量则用术语“读”和“更改”

![image-20230704192828718](UNIX编程·进程与线程.assets/image-20230704192828718.png)
...
- 这些 IPC 结构在文件系统中没有名字，为支持这些 IPC对象，内核中增加了十几个全新的系统调用来访问它们或者修改它们的属性。
- 因为这些形式的 IPC 不适用文件描述符，所以不能对他们使用多路转接 I/O 函数（select 和 epoll ），这使得它很难一次使用一个以上这样的 IPC结构，或者在文件或设备 I/O 中使用这样的 IPC结构。

优点是：

它们是可靠的、流控制的以及面向记录的；它们可以用**非先进先出次序**处理 。

## 消息队列

**消息队列**（队列）是消息的链接表，存储在内核中，由**消息队列标识符**（队列ID）标识。
...
struct msqid_ds {
    ...
    // ... other implementation-specific fields may follow
};
```

对它的操作有这些函数：

#### 用于创建一个新队列或打开一个现有队列——`msgget`函数
...
`key`：一个 System V IPC 键值，用于标识消息队列。通常使用 `ftok` 函数将路径名和项目 ID 转换成一个键值。

`flag`：标志位，用于指定创建消息队列的方式和访问权限等信息。可以使用 IPC_CREAT 标志来创建一个新的消息队列，使用 IPC_EXCL 标志来指定只创建新队列，如果队列已经存在则返回错误。还可以使用 IPC_PRIVATE 标志来创建一个私有的消息队列，只允许当前进程访问。可以使用 IPC_PERM 标志来指定访问权限。

**返回值**

成功，返回一个 System V 消息队列的标识符（非负整数）。

失败，则返回 -1，并设置 `errno` 错误码。

#### 将新消息添加到队列尾端——`msgsnd`函数

每个消息包含一个正的长整型类型的字段、一个非负的长度以及实际数据字节数（对应于长度），所有这些都在将消息添加到队列时，传送给`msgsnd`。
...
`ptr`: 用于存储接收到的消息数据的缓冲区指针

`nbytes`: 缓冲区的大小，即可接收的最大字节数

`type`: 消息类型，指定要接收的消息的类型，如果type为0，则接收队列中的第一个消息

`flag`: 接收消息的标志位，可为0或IPC_NOWAIT。如果flag为0，则函数将阻塞等待消息到达消息队列；如果flag为IPC_NOWAIT，则函数立即返回，如果队列中没有符合要求的消息，则返回-1并将errno设置为ENOMSG。

#### 对队列执行多种操作——`msgctl` 函数

（ 取队列的 `msqid_ds` 结构、删除消息队列等 ）
...
接收者可以使用消息类型以非先进先出的次序取消息 。

**参数**

`msqid`: 消息队列的标识符

`cmd`: 控制命令，指定要执行的操作，可以是IPC_STAT、IPC_SET、IPC_RMID之一，分别表示获取消息队列的状态信息、设置消息队列的状态信息、删除消息队列

`buf`: 指向msqid_ds结构体的指针，用于传递或获取消息队列的状态信息
...
信号量与已经介绍过的 IPC 机构（管道、FIFO 以及消息列队）不同，它是一个计数器，用于为多个进程提供对共享数据对象的访问，信号量通常在内核中实现 。
...
**信号量集合**有如下结构，名为`semid_ds`，被内核维护着
...
成功时返回信号量集标识符，失败返回-1，并设置`errno`变量以指示错误类型

参数:

`key`: 用于标识信号量集的键值，与`semop`等函数使用的相同

`nsems`: 信号量集中包含的信号量数目

`flag`: 标志位，指定信号量集的创建和访问权限，可以使用IPC_CREAT、IPC_EXCL等常量，与`semget`函数的flag参数使用的相同。
...
控制信号量集，此函数包含了删除信号量集合、获取集合 **semid_ds** 结构、设置 **sem_perm** 属性 、返回信号量各属性等功能 

**返回值**

成功时返回0，失败返回-1，并设置errno变量以指示错误类型

**参数**

`semid`: 信号量集的标识符

`semnum`: 信号量集中要操作的信号量的编号

`cmd`: 控制命令，指定要执行的操作，可以是IPC_STAT、IPC_SET、IPC_RMID、GETVAL、SETVAL等常量之一，分别表示获取信号量集的状态信息、设置信号量集的状态信息、删除信号量集、获取信号量的值、设置信号量的值等操作

`arg`: 用于传递额外参数的联合体，具体参数的类型和含义取决于cmd参数，通常是一个semun联合体
...
`semoarray` ：指向一个结构体数组的指针，每个结构体描述一个对信号量的操作，包括信号量编号、操作类型和操作方式，具体定义如下：

```
struct sembuf {
    unsigned short sem_num;  /* 信号量编号 */
    short          sem_op;   /* 操作类型和操作方式 */
    short          sem_flg;  /* 操作标志，通常为SEM_UNDO */
};
```

`nops`: 要进行的信号量操作的数量，即`semoarray`数组中元素的数量

结构体的`sem_flg` 中，`IPC_NOWAIT` 用于指定函数是否阻塞 ；若对信号量操作设了 `SEM_UNO` ，然后分配资源（ `sem_op` 值小于 0 ），那么内核就会记住对于该特定信号量，分配给调用进程多少资源（ `sem_op` 的绝对值 ）。当该进程终止时，内核将检验该进程是否还有尚未处理的 信号量调整值 ，如果有，则按调整值对相应信号量值进行处理 。

对集合中每个成员的操作 由 `sem`_op 规定，此值可以是负值、0 或正值 ：

- 若为 正值 ，对应于进程释放的占用的资源数 。`sem_op` 值会加到信号量的值上 。如果指定了 `undo` 标志，则也从该进程的此信号量调整值中减去 `sem_op` 。
- 若为 负值 ， 对应于进程获取由该信号量控制的资源 。 若信号量的值大于等于 `sem_op` 的绝对值，则从信号量值中减去 `sem_op` 的绝对值 。若指定了 `undo` 标志，此信号量调整值加上 `sem_op` 的绝对值 。
- 若为 0 ，表示调用进程希望等待到该信号量值变成 0 。

#### 通用的使用方法

可以使用 `semget` 函数来创建一个信号量集合，该集合包含若干个信号量。然后使用 `semctl` 函数将这些信号量的值初始化为 1，表示资源可用。

为了分配资源，我们可以使用 `semop` 函数，将某个信号量的值减 1，表示该资源被占用。而为了释放资源，我们可以再次使用 `semop` 函数，将该信号量的值加 1，表示该资源现在可用。

为了处理在未释放资源条件下进程终止的情况，在每个 `semop` 操作中都应该指定 `SEM_UNDO` 标志，这样在进程终止时，内核会自动将该进程持有的所有信号量的值恢复到原来的状态。

### 共享存储

共享存储机制允许两个或多个进程共享同一个给定的存储区，而不需要在客户进程和服务器进程之间复制数据，因此是一种最快的IPC机制。使用共享存储区时，需要进行同步以确保多个进程可以同时访问给定存储区。通常，信号量机制用于同步共享存储访问。

与内存映射文件不同，XSI共享存储段不需要与文件关联。XSI共享存储段是一块匿名的内存空间，可以用于多个进程之间共享数据。在使用共享存储机制时，需要使用信号量机制进行同步，以确保多个进程可以安全地访问共享存储区。
...
成功时返回共享存储段的标识符，失败返回-1，并设置errno变量以指示错误类型

**参数**

`key`: 用于标识共享存储段的键值，与`shmat`等函数使用的相同，指明是创建一个新共享存储段，还是引用一个现有的共享存储段 。

`size`: 共享存储段的大小，以字节为单位。参数是该共享存储段的长度，以字节为单位，通常将其向上取为系统页长的整数倍 。如果正在创建一个新段（通常在服务器进程中），则必须指定其 size 。如果正在引用一个现存的段（一个客户进程），则将 size 指定为 0 。当创建一个新段时，段内的内容初始化为 0 。

`flag`: 标志位，指定共享存储段的创建和访问权限，可以使用IPC_CREAT、IPC_EXCL等常量，与`shmget`函数的flag参数使用的相同。

#### 对共享存储段执行多种操作——`shmctl`函数
...
`cmd`: 控制命令，指定要执行的操作，可以是IPC_STAT、IPC_SET、IPC_RMID等常量之一，分别表示获取共享存储段的状态信息、设置共享存储段的状态信息、删除共享存储段、取此段的 **`shmid_ds`** 结构、设置 **`shmid_ds`** 字段、从系统中删除共享存储段、在内存中对共享存储段加锁 、解锁共享存储段（最后两个命令只能由超级用户执行） 等 。

`buf`: 指向`shmid_ds`结构体的指针，用于传递额外参数，具体参数的类型和含义取决于`cmd`参数

#### 如果进程调用，将共享存储段连接到它的地址空间中—— `shmat` 函数
...
- 成功时返回指向共享存储段的指针
- 失败时返回-1，并设置errno变量以指示错误类型

**参数**

`shmid`: 共享存储段的标识符

`addr`: 指定共享存储段连接到进程的地址，通常为NULL，表示让内核选择一个合适的地址。若 `addr` 为 0 ，则此段连接到由内核选择的第一个可用地址上（推荐） 。若不为 0 ，结合 flag 参数的 `SHM_RND` 可指定连接地址 。

`flag`: 标志位，指定共享存储段的访问权限，可以使用SHM_RDONLY等常量，与`shmat`函数的flag参数使用的相同。如果在 flag 中指定了 `SHM_RDONLY`  位，以只读方式连接此段，否则以读写方式连接此段 。

如果 `shmat` 成功执行，那么内核将使与该共享存储段相关的 `shmid_ds` 结构中的 `shm_nattch` 计数器加 1 。进程可以对函数返回指针指向的地址进行读或写操作 。

![image-20230704220533132](UNIX编程·进程与线程.assets/image-20230704220533132.png)

#### 对共享存储段的操作结束时，进程调用分离此段——`shmdt` 函数
...
**`addr`**: 指向共享存储段连接到进程的地址的指针，这是是以前调用 `shmat` 时的返回值 。如果成功，`shmdt` 将使相关 `shmid_ds` 结构中的 `shm_nattch` 计数器值减一 。

`shmdt` 函数的分离操作并不意味着从系统中删除其标识符以及相关的数据结构 。该标识符仍然存在，直至某个进程（一般是服务器进程）以 `IPC_RMID` 命令的调用 **`shmctl`** 函数特地删除它为止 。

###  POSIX 信号量

#### 概念

这是`POSIX`标准下的信号量，POSIX 信号量意在解决 XSI 信号量 的几个缺陷：
...
它分为两类，命名信号量 和 未命名的信号量（差异在于创建和销毁形式）：
...
##### 创建一个新的命名信号量 / 使用一个现有信号量—— `sem_open` 函数
...
成功时返回指向信号量的指针，失败时返回 SEM_FAILED，并设置`errno`变量以指示错误类型

**参数**

`name`: 信号量的名称，以“/”开头，类似于路径名

`oflag`: 标志位，指定创建或打开信号量的方式，可以使用O_CREAT、O_EXCL等常量，与open函数的`oflag`参数使用的相同

`mode`: 权限位，指定新创建的信号量的访问权限，只有在`oflag`包括O_CREAT时才有效，与open函数的mode参数使用的相同

`value`: 信号量的初始值，只有在`oflag`包括O_CREAT时才有效

##### 释放信号量相关的资源—— `sem_close` 函数
...
关闭命名信号量。如果进程没有首先调用 **sem_close** 而退出，那么内核将自动关闭任何打开的信号量 。这不会影响信号值量的状态 。类似地，如果调用 **sem_close** ，信号值也不会受影响 。
...
函数删除信号量的名字 。如果没有打开的信号量引用，额该信号量会被销毁 。否则，销毁将延迟到最后一个打开的引用关闭 。

**`sem_unlink`** 是解除进程和信号量的关联；**`sem_close`** 是释放信号量相关资源 。
...
`name`: 信号量的名称，与`sem_open`函数的`name`参数使用的相同

##### 信号量的减一操作—— `sem_wait/ sem_trywait / sem_timewait` 函数
...
`tsptr`: 指向`timespec`结构体的指针，表示等待时间的长度和精度
...
`sem_wait`操作信号量，将信号量的值减1，如果信号量的值为0，则阻塞，直到成功将信号量的值减1或者被信号中断；

 `sem_trywait`操作信号量，将信号量的值减1，如果信号量的值为0，则立即返回-1，不阻塞；

`sem_timedwait`操作信号量，将信号量的值减1，如果信号量的值为0，则等待一段时间，如果在规定时间内成功将信号量的值减1或者被信号中断，则返回0，否则返回-1。
...
初始化未命名信号量,需要声明一个 `sem_t` 类型的变量并把它的地址传递给 `sem_init` 来实现初始化 。如果在两个进程之间使用信号量，需要确保 `sem` 参数指向两个进程之间共享的内存范围 。
...
`pshared`:指定信号量的共享方式，可以是0或者非0，0表示信号量只能在调用进程内部使用，非0表示信号量可以在多个进程之间共享

`value`:指定信号量的初始值

##### 丢弃未命名信号量——`sem_destroy` 函数
...
`valp`: 指向int型变量的指针，用于存储获取到的信号量的值

## 几种 IPC 的应用

管道和FIFO仍然是很常用的进程间通信方式，它们适用于很多应用程序场景。在使用管道的模型中，客户进程使用fork和exec调用启动服务器进程来执行特定任务，例如打开文件。服务器进程可以是一个设置用户ID的程序，这使它具有特殊的权限，可以决定客户进程的真实身份。

对于新的应用程序，可以考虑使用全双工管道和记录锁来进行进程间通信，它们使用起来更加方便。全双工管道提供了双向通信的能力，而记录锁可以用于同步和互斥。

消息队列和信号量虽然也可以用于进程间通信，但是它们的使用比较繁琐，而且容易出错。因此，在新的应用程序中，应尽可能避免使用消息队列和信号量。

共享存储仍然有它的用途，可以使用`mmap`函数来实现共享存储，它提供了一种简洁而精确的方式来共享内存。但是，使用共享存储时需要注意进程间同步的问题，否则可能会出现数据一致性问题。

另一种类型的服务器进程是守护进程，客户进程通过一种命名IPC（如FIFO或消息队列）与其通信。使用FIFO时，每个客户进程需要一个单独使用的FIFO，而在使用消息队列时，可以使用一个众所周知的队列并使用消息类型字段来区分不同的客户进程。使用这些IPC类型时，需要考虑如何确保服务器进程可以准确标识客户进程，以便进行身份验证。

为了实现身份验证，可以使用FIFO、消息队列、信号量或共享存储，并使用`ipc_perm`结构中的相关字段来标识IPC结构的创建者和访问权限。另一种更好的方法是使用套接字子系统，在传送文件描述符的过程中获取客户进程的有效用户ID和有效组ID。
